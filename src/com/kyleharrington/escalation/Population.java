...
transient XYSeriesCollection xyColl = new XYSeriesCollection();
	...
	public void init() throws Exception {
		...
		directoryName = UID; // this was the way it was set for the main runs, changed for knockout #UPDATE, undone
...
public void evolve( long numSteps ) throws Exception {
	...
	public void evolve_resume( long numSteps ) throws Exception {
		...
		try {
			...
			// Now for knockout #UPDATE
			String pop_fn = workingDirectory + "/" + "population_MaxMem_" + maxMemory + "_randomSeed_" + randomSeed + "_UID_" + UID + "_T_" + t + ".ser";
			String rng_fn = workingDirectory + "/" + "RNG_MaxMem_" + maxMemory + "_randomSeed_" + randomSeed + "_UID_" + UID + "_T_" + t + ".ser";
...
//			System.err.print("\t" + args[k]);
			...
			//} else if ( args[k].compareTo("runID") == 0 ) {
				...
				System.out.println( "wd = " + p.workingDirectory );
			...
			/*} else if ( args[k].compareTo("extinctionThreshold") == 0 ) {			
				...
				System.out.println( "Knockout species: " + p.knockoutSpecies );
			}
			//public double[] scores1 = { 1.0, 5.0, 0.0, 3.0 };// Scores for player1
			//public double[] scores2 = { 1.0, 0.0, 5.0, 3.0 };// Scores for player 2*/						
		}			
		...
		if( resumeFilename == "" ) {
			...
			if( resumePopFilename == "" || resumeRNGFilename == "" ) {
				...
				String loadedDirname = p.workingDirectory;
				int knockoutSpecies = p.knockoutSpecies;
				p = loadPopulation( directory + "/" + resumePopFilename );
				p.RNG = loadRNG( directory + "/" + resumeRNGFilename );
				p.directoryName = loadedDirname;//p.workingDirectory;// Changed for knockout #UPDATE
				p.workingDirectory = loadedDirname;
				p.enablePlotting = false;
				System.out.println( p.workingDirectory );
				File outDir = new File( p.directoryName );
				if (!outDir.exists()) outDir.mkdir();  
				
				p.init_stats();
				p.directoryName = p.workingDirectory;// Changed for knockout #UPDATE
				System.out.println("Population loaded from file. Current generation: " + p.t + " Max gens is: " + maxGens);
				p.knockoutSpecies = knockoutSpecies; 
			}
			
			// Report before knockout
			p.report();
			
			if( p.knockoutSpecies >= 0 ) {// CHanged for knockout #UPDATE
				System.out.println( "Knocking out species: " + p.knockoutSpecies );
				// Reduce matrices			
		    	...
		    	for( int r = 0; r < p.payoff.rows; r++ ) {
		    		for( int c = 0; c < p.payoff.columns; c++ ) {
		    			...
		    			if( r != p.knockoutSpecies && c != p.knockoutSpecies ) {	    				
		    				if( r > p.knockoutSpecies ) {
		    					thisR--;
		    				} 
		    				if( c > p.knockoutSpecies ) {
		    					thisC--;
		    				}
		    	...
		    	double knockoutFrequency = p.genotype.get( p.knockoutSpecies ).frequency;
		    	
		    	p.genotype.remove( p.knockoutSpecies );
		    	p.numSpecies--;
		    	
		    	// Redistribute to species randomly
		    	for( double redist = 0; redist < knockoutFrequency; redist += p.extinctionThreshold ) {
		    		int r = p.RNG.nextInt( p.numSpecies );
		    		p.genotype.get( r ).frequency += p.extinctionThreshold;
		    	}
		    	
		    	p.computeFitness();// lazy way
		    	
		    	p.report();// Report after knockout
			}
